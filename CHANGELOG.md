...
- Private vulnerability reporting (Settings -> Security -> Private
  vulnerability reporting) needs enabling by hand too. `SECURITY.md`
  documents it as the disclosure channel, but the toggle isn't on by
  default, so it needs confirming before that channel actually exists.
