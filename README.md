...
`policy-command` runs as raw shell text, the same trust level `extra-args`
already has elsewhere in this action. Keep it static workflow config.
Never build it from PR-controlled input like a title, label, or comment,
there's no quoting or sanitization boundary here.
