...
# extra-args must reach both internal plan calls but never the final apply.
extra_args_plan_cmd="$(build_plan_command "tofu" "/tmp/runner-temp" "-var-file=foo.tfvars")"
assert_contains \
  "extra-args reaches the plan step's plan command" \
  "$extra_args_plan_cmd" \
  "-var-file=foo.tfvars"

extra_args_apply_plan_cmd="$(build_apply_plan_command "tofu" "/tmp/runner-temp" "-var-file=foo.tfvars")"
assert_contains \
  "extra-args reaches the apply step's internal plan command" \
  "$extra_args_apply_plan_cmd" \
  "-var-file=foo.tfvars"
